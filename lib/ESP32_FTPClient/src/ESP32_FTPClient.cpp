...
// FitaDigital: emite "SIZE <path>" no socket de CONTROLO e devolve o tamanho
// remoto (resposta "213 <n>"), ou -1 se nao suportado / ficheiro ausente / erro.
long ESP32_FTPClient::Size(const char * fileName) {
  FTPdbgn(F("Send SIZE"));
  if(!isConnected()) return -1;

  client.print(F("SIZE "));
  client.println(fileName);   // println acrescenta CRLF

  // Le a resposta do socket de controlo.
  unsigned long _m = millis();
  while(!client.available() && millis() < _m + timeout) delay(1);
  ...
  while(client.available()) {
    char c = client.read();
    resp += c;
    if(c == '\n') break;       // primeira linha basta
  }

  // Espera "213 <n>".
  int idx = resp.indexOf("213");
  if(idx < 0) return -1;
  idx += 3;
  while(idx < (int)resp.length() && resp[idx] == ' ') idx++;
  if(idx >= (int)resp.length() || resp[idx] < '0' || resp[idx] > '9') return -1;
  return strtol(resp.c_str() + idx, NULL, 10);
}
