...
void emit_cycle_close(const Current &snap, CycleDetectorStatus st) {
  ...
  app_log_feature_writef("INFO", "CYCLE", "fechado status=%s lines=%u bytes=%u dur=%us",
                         status_str(st), (unsigned)snap.line_count,
                         (unsigned)snap.byte_count, (unsigned)dur);
}
...

  {
    ...
    if (s_cur.state == State::IDLE) {
      if (has_start) {
        ...
        app_log_feature_writef("INFO", "CYCLE", "iniciado @ %ld first='%.40s'",
                               (long)now, line);
      }
...
void cycle_detector_tick(time_t now) {
...

  {
    Lock lk;
    if (!s_cfg.enabled) return;
    if (s_cfg.idle_timeout_s == 0) return;
    if (s_cur.state != State::ACTIVE) return;
    const time_t elapsed = now - s_cur.last_line_ts;
    if (elapsed < (time_t)s_cfg.idle_timeout_s) return;
    snap_to_emit = s_cur;
    emit = true;
    mark_idle();
  }
