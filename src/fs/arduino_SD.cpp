...
bool SDFS::begin(uint8_t ssPin, SPIClass &spi, uint32_t frequency, const char *mountpoint, uint8_t max_files, bool format_if_empty) {
  ...
  /* CS no CH422G: pinos SPI do TF na 4.3B (Waveshare); ordem ESP32: begin(sck, miso, mosi, ss). */
  if (ssPin == 0xFF) {
    spi.begin(BOARD_TF_SPI_SCK, BOARD_TF_SPI_MISO, BOARD_TF_SPI_MOSI, -1);
  } else {
    spi.begin();
  }
