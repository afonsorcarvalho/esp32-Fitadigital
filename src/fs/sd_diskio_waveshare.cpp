...
/** CS no CH422G (ssPin == 0xFF); caso contrário GPIO Arduino (stock). */
static inline void sd_cs_high(ardu_sdcard_t *card) {
  if (card->ssPin == 0xFF) {
...
static inline void sd_cs_low(ardu_sdcard_t *card) {
  if (card->ssPin == 0xFF) {
    waveshare_sd_cs_set(false);
  } else {
...
bool sdcard_format(uint8_t pdrv) {
  ardu_sdcard_t *card = s_cards[pdrv];
  if (pdrv >= FF_VOLUMES || card == NULL || card->fs == NULL) {
    return false;
  }
