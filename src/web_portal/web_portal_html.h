...
<button type="button" data-p="cyc">Ciclo</button>
...
<section id="p-cyc" class="panel"><h2>Detector de Ciclos</h2>
<div id="cycMsg"></div>
<div class="card"><h2>Parser RS485</h2>
<p style="font-size:13px;opacity:.85">Patterns case-insensitive (substring). Vazio em "Inicio" desactiva detector.</p>
<label>Inicio (ex: OPERACAO)</label><input id="cycSt" maxlength="47" autocomplete="off">
<label>Fim (ex: FIM CICLO)</label><input id="cycEn" maxlength="47" autocomplete="off">
<label>Timeout idle (segundos, 0=sem timeout, max 86400)</label><input id="cycTo" type="number" min="0" max="86400">
<button class="btn" type="button" id="btnCycSave">Guardar</button>
<div id="cycStatus" style="margin-top:10px;font-size:13px;opacity:.85"></div>
</div>
</section>
...
function loadCycCfg(){
api("GET","/api/cycles/config").then(function(d){
$("cycSt").value=d.startPattern||"";
$("cycEn").value=d.endPattern||"";
$("cycTo").value=(d.idleTimeoutS!=null)?d.idleTimeoutS:900;
$("cycStatus").textContent=d.enabled?"Detector activo.":"Detector desactivado (inicio vazio).";
}).catch(function(e){$("cycMsg").innerHTML='<div class="msg err">'+e+'</div>';});
}
$("btnCycSave").onclick=function(){
var body={
startPattern:$("cycSt").value,
endPattern:$("cycEn").value,
idleTimeoutS:parseInt($("cycTo").value,10)||0
};
$("cycMsg").innerHTML="";
api("POST","/api/cycles/config",body).then(function(r){
$("cycMsg").innerHTML='<div class="msg ok">Guardado.</div>';
var a=r.applied||{};
$("cycStatus").textContent="Aplicado: "+(a.enabled?"activo":"desactivado")+
" (start=\""+(a.startPattern||"")+"\" end=\""+(a.endPattern||"")+"\" idle="+(a.idleTimeoutS||0)+"s)";
}).catch(function(e){$("cycMsg").innerHTML='<div class="msg err">'+e+'</div>';});
};
function connectLog(){
disconnectLog();
