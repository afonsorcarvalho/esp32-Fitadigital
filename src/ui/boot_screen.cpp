...
static void apply_tag_color(lv_obj_t *tag_lbl, const char *level) {
  ...
  lv_color_t c = lv_color_hex(0x00CC66);
  if (level != nullptr && strcmp(level, "ERROR") == 0) {
    c = lv_color_hex(0xFF6644);
  } else if (level != nullptr && strcmp(level, "WARN") == 0) {
    c = lv_color_hex(0xFFCC33);
  } else if (level == nullptr) {
    c = lv_color_hex(0x669988);
  }
...
void boot_screen_show(void) {
  ...
  lv_obj_set_style_text_color(s_subtitle, lv_color_hex(0x88AA99), LV_PART_MAIN);
  ...
  for (int i = 0; i < (int)BOOT_STEP_COUNT; ++i) {
    ...
    lv_obj_set_style_text_color(left, lv_color_hex(0x00CC66), LV_PART_MAIN);
    ...
    lv_obj_set_style_text_color(dots, lv_color_hex(0x508878), LV_PART_MAIN);
  ...
  lv_obj_set_style_text_color(s_footer, lv_color_hex(0x558877), LV_PART_MAIN);
